...
import {
    OrderBy,
    useGetProductItemsGroupedByOrderCodeQuery,
    useGetListingOrdersQuery,
...
import ModalContent from '@/components/OrderManagement/ModalContent';
import { useTranslations } from 'next-intl';
import ModalDataService from '@/components/OrderManagement/ModalDataService';
type Order = NonNullable<GetListingOrdersQuery['listingOrders'][0]>;
const DataModal = ({
    order,
    onClose
}: {
    ...
    const { data: orderDetails, loading: orderDetailsLoading } =
        useGetProductItemsGroupedByOrderCodeQuery({
            variables: {
                userId: session?.user?.id,
                orderId: order.orderId,
                productId: order.productId,
                saleAt: order.orderDate
            }
        });

    const orderDetailsData =
        orderDetails?.orders?.[0]?.product?.productItems || [];
            ...
            <div className="flex flex-col gap-4">
                <div className="">
                    {order.orderType === 'service' ? (
                        <ModalDataService testMode={false} orderData={{
